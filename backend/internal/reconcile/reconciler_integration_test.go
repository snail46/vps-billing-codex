...
func TestProcessorRecoversDurableStateAndSchedulesDriftOperation(t *testing.T) {
	...
	fixtures := []struct {
		...
		{`INSERT INTO nodes(id,provider_id,node_group_id,provider_node_id,name,region,status,cpu_total,memory_total_mb,disk_total_gb,cpu_reserved,memory_reserved_mb,disk_reserved_gb,capabilities,last_seen_at) VALUES($1,$2,$3,$1::text,$4,'test','online',8,8192,200,1,1024,20,'{"virtualization":"kvm"}',now())`, []any{nodeID, providerID, groupID, "reconcile-node-" + nodeID.String()}},
		{`INSERT INTO nodes(id,provider_id,node_group_id,provider_node_id,name,region,status,cpu_total,memory_total_mb,disk_total_gb,capabilities,last_seen_at) VALUES($1,$2,$3,$1::text,$4,'test','online',8,8192,200,'{"virtualization":"kvm"}',now()-interval '10 minutes')`, []any{offlineNodeID, providerID, groupID, "offline-node-" + offlineNodeID.String()}},
	...
	assertValue(t, pool, `SELECT cpu_reserved::text||':'||memory_reserved_mb::text||':'||disk_reserved_gb::text FROM nodes WHERE id=$1`, nodeID, "0:0:0")
