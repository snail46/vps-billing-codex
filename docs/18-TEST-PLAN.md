...
Release Gate：验证生产配置 fail-closed、安全响应头、受保护 Metrics、全部 12 组 migration up/down 配对、部署脚本语法、npm audit、Admin TOTP 服务与双语 UI，以及完整 backend race test、frontend lint/test/typecheck/build、Compose 购买/支付/开通/实例动作/Admin Audit 流程。Compose Gate 还必须实际生成加密 PostgreSQL/配置备份、校验 checksum、停止写入者、恢复数据与配置、重新启动并通过 health/metrics。Migration 12 为 Outbox、失败 Operation、成功 Payment、Audit、Ticket 与未读 Notification 热路径提供索引。

V2 Gate expands migration coverage to 19 pairs and the E2E chain to NAT add/delete, usage ingestion/rating, renewal, promotion snapshot, refund reversal, dead-letter/replay and billing-profile invoice snapshot. Exact minor-unit tests must cover overflow and rounding. The quantitative failure matrix is maintained in `docs/23-V2-OPERATIONS-AND-FAILURE-MATRIX.md`; release evidence records actual counts rather than checking boxes without execution.
